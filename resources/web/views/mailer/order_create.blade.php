...
<div style="width: 680px;">
    <div style="text-align:center;margin-bottom: 15px;">
        <a href="{{ getConfigDb('config_storage_domain') }}" title="{{ getConfigDb('config_name') }}">
            <img src="{{ resizeImage(getConfigDb('config_logo'), 140, 50, 'client') }}"
                 alt="{{ getConfigDb('config_name') }}"
                 style="margin-bottom: 20px; border: none;"/>
        </a>
    </div>
    <p style="margin-top: 0px; margin-bottom: 20px;">
        Cám ơn bạn đã đặt hàng tại {{ getConfigDb('config_name') }}. Đơn hàng của bạn đã được ghi nhận và sẽ được xử lý
        ngay.
    </p>

    <p style="margin-top: 0px; margin-bottom: 20px;">Để theo dõi tình trạng đơn hàng bấm vào đường dẫn bên dưới:</p>
    <p style="margin-top: 0px; margin-bottom: 20px;">
        <a href="{{ getConfigDb('config_storage_domain').'/order/search?order_code='.$infoCustomer['uniqid'] }}">
            {{ getConfigDb('config_storage_domain').'/order/search?order_code='.$infoCustomer['uniqid'] }}
        </a>
    </p>
    ...
    <table
        ...
        <tr>
            <td style="font-size: 12px;	border-right: 1px solid #DDDDDD; border-bottom: 1px solid #DDDDDD; text-align: left; padding: 7px;">
                <b>Mã đơn hàng:</b> {{ getConfigDb('config_invoice_prefix') .'-'. $infoCustomer['uniqid'] }}<br/>
            ...
            <td style="font-size: 12px;	border-right: 1px solid #DDDDDD; border-bottom: 1px solid #DDDDDD; text-align: left; padding: 7px;">
                ...
                <b>Tình trạng đơn hàng:</b> {{ $infoCustomer['order_status'] }}<br/></td>
        </tr>
        </tbody>
    </table>
    <table
