...
@section('style')
    <link rel="stylesheet" href="{{ publicUrl('web/css/theme-aurora-product-detail.css?v=' . getConfigDb('config_theme_version')) }}">
@stop
@section('meta')
    <!-- META FOR FACEBOOK -->
...
@stop
@section('content')
    <div class="mx-auto max-w-[1320px] px-5" x-data="{ contactOpen: false, tab: 'description' }">
        <nav class="py-4 text-[13px] text-ink-2 flex items-center gap-2 flex-wrap">
            @foreach ($breadcrumbs as $i => $breadcrumb)
                ...
                @else
                    <span class="text-ink truncate">{{ $breadcrumb['text'] }}</span>
                @endif
            @endforeach
        </nav>
        <div class="rounded-3xl bg-white border border-line p-5 lg:p-8 mb-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="mb-md-0 mb-sm-5">
                        <div class="detail-info">
                            ...
                            <h1 class="text-[24px] lg:text-[28px] font-extrabold tracking-tight leading-[1.2] mt-1">
                                {{ $entity->name }}
                            </h1>
                            @php
        ...
        <section class="pb-10">
            <div class="flex gap-1.5 border-b border-line mb-6 overflow-x-auto">
                <button @click="tab = 'description'"
                    class="shrink-0 h-11 px-5 text-[15px] font-semibold border-b-2 -mb-px transition"
                    :class="tab === 'description' ? 'border-ink text-ink' : 'border-transparent text-ink-3 hover:text-ink'">
                    Mô tả
                </button>
                @if ($entity->isReview)
                    <button @click="tab = 'reviews'"
                        class="shrink-0 h-11 px-5 text-[15px] font-semibold border-b-2 -mb-px transition"
                        :class="tab === 'reviews' ? 'border-ink text-ink' : 'border-transparent text-ink-3 hover:text-ink'">
                        Đánh giá ({{ number_format($entity->reviewCount ?? 0) }})
                    </button>
                @endif
            </div>

            <div x-show="tab === 'description'" class="max-w-[70ch] text-[15.5px] leading-[1.75] text-ink-2">
        ...
        @if (count($relatedProducts))
            <section class="pb-12">
                <h2 class="text-[24px] font-bold tracking-tight mb-4">Có thể bạn muốn xem?</h2>
        ...
        @if (count($latestBlogs))
            <section class="pb-12">
                <h2 class="text-[24px] font-bold tracking-tight mb-4">Chia sẻ</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    @foreach ($latestBlogs as $blog)
                        <a href="{!! $blog->url !!}" title="{!! $blog->title !!}"
                            ...
                            <div class="p-4">
                                <p class="text-[15px] font-semibold leading-snug line-clamp-2 group-hover:text-brand transition">
                                    {{ $blog->title }}
                                </p>
                                <p class="text-[12.5px] text-ink-3 mt-2">
                                    {!! $blog->publishedDate !!} · {{ $blog->viewed }} lượt xem
                                </p>
                            </div>
                        </a>
                    @endforeach
